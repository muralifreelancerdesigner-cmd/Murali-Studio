...
export default function Hero() {
  return (
    <section className="relative w-full max-w-5xl mx-auto pt-32 pb-20 px-6 flex flex-col items-center text-center z-10">
      ...
      {/* Main Punchy Heading */}
      <h1 className="text-4xl sm:text-6xl font-extrabold text-neutral-100 tracking-tight leading-[1.15] max-w-3xl block">
      ...
      {/* Profile Explicit Context Bio */}
      <p className="text-base sm:text-lg text-neutral-400 mt-6 max-w-2xl leading-relaxed block">
        I am <strong className="text-neutral-200 font-semibold">{PROFILE_INFO?.name || "R. Murali"}</strong>, a senior <span className="text-emerald-400">{PROFILE_INFO?.title || "Multimedia Professional"}</span> based in {PROFILE_INFO?.location || "Chennai, India"}. With over a decade of hands-on experience, I bridge the gap between high-end digital design and multi-platform interactive application loops.
      </p>
